...
void StiAnalyzer::BookHistograms()
{
  ...
  for(int etabin = 0; etabin < numEtaBins; etabin++)
  {
    ...
    pullEtaAcc[etabin] = new TH1F(Form("pullEtaAcc_%i",etabin),Form("Pull for %.2f < #eta < %.2f; Pull (cm/#sigma); Counts",etaBinLow[etabin],etaBinHigh[etabin]),200,0,10);
    pullEtaRej[etabin] = new TH1F(Form("pullEtaRej_%i",etabin),Form("Pull for %.2f < #eta < %.2f; Pull (cm/#sigma); Counts",etaBinLow[etabin],etaBinHigh[etabin]),200,0,10);
  ...
  for(int ptbin = 0; ptbin < numPtBins; ptbin++)
  {
    ...
    pullPtAcc[ptbin] = new TH1F(Form("pullPtAcc_%i",ptbin),Form("Pull for %.2f < pT < %.2f; Pull (cm/#sigma); Counts",ptBinLow[ptbin],ptBinHigh[ptbin]),200,0,10);
    pullPtRej[ptbin] = new TH1F(Form("pullPtRej_%i",ptbin),Form("Pull for %.2f < pT < %.2f; Pull (cm/#sigma); Counts",ptBinLow[ptbin],ptBinHigh[ptbin]),200,0,10);
